...
function Professor() {
    ...
    async function handleAddStudent() {
        const AddStudentDialog = () => {
            ...
            return (
                <>
                    <input ref={ref} type="text" placeholder="Nome do aluno" className="p-2 rounded-md border-2 border-gray-300" />
                    <button
    ...
    const ListItem = ({ item, checked, onClick }: { item: string; checked?: boolean, onClick?: () => void }) => (
        <div className={
            checked
    ...
    const List = () => (
        <div className="flex flex-col items-stretch gap-2 justify-start w-full p-2 pt-4 pb-20 overflow-y-auto">
            <>
                {classe?.alunos && Object.entries(classe.alunos).map(([key, aluno]) => (
                    <ListItem
                        key={key}
                        item={aluno}
